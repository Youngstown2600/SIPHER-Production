...
grep -v -E '  (scripts/(build-pjsip\.sh|build-macos\.sh|build-termux\.sh)|include/trunkmonkey/(PbxAudit|SipTrace|SipWireMonitor)\.h|src/core/(PbxAudit|SipTrace|SipWireMonitor)\.cpp)$' \
...
find src/core include/trunkmonkey scripts -type f ! -path '*/Version.h' \
  ...
  ! -path 'include/trunkmonkey/PbxAudit.h' ! -path 'include/trunkmonkey/SipTrace.h' ! -path 'include/trunkmonkey/SipWireMonitor.h' \
